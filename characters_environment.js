...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    //👇(142)Cambia "presiona cualquier flecha" a "Haz click en Jugar para iniciar el juego"
...
// Cambiar la declaración de  iniciar si se preciona cualqueir tecla  a las condiciones de posicion de nariz, Game config y Game status 👇
function changeGameStatud(character){
 ...
 { 
   ...
   world_start.play();//👈(142)VERIFICAR QUE ESTÉ ESCRITO  world_start.play();
...
function getCoins(coin,character){
  ...
  /*⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐
  👇(143) declara la función para reproducir audio al obtener una moneda*/
    mario_coin.play();
  };
}
...
function manualControl(character){
  ...
  if(character.live){
    //👇(142)cambiar "keyDown(control.left) por (noseX<300)"
    if(noseX < 300){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    ...
    //👇(142)cambiar "keyDown(control.right) por (noseX>300)"
    if(noseX > 300){
        character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    ...
    //👇(142)cambiar "keyDown(control.left) por (noseX<300)" , "keyDown(control.right) y "keyDown()" por :
    if(
      !(noseX < 300)
      &&!(noseX > 300)
      &&!(noseY<200)
...
function jumping(character){
  //👇(142)cambiar keyDown(control.up) por "noseY < 200"
	if( (noseY < 200  
    &&character.live) || (touchIsDown&&character.live) ){
    character.velocity.y+=gameConfig.jump;
    //👇(143) declarar la función para reproducir la música al saltar
    mario_jump.play();
	}
}
...
function StepOnEnemy(obj1,obj2){
    ...
    //👇(143)Declarar la función para reproducir el efecto de sonido.
    mario_kick.play();
	}
}
...
function die(character){
    ...
    {
      //👇(143)declara la función para reproducir el efecto de sonido
      mario_die.play();
    }
}
...
function checkStatus(character){    
  ...
  {
    //👇verifica que la función Gameconfig cambie a gameover y declara la función para reproducir su efecto de sonido.
